...
## 支持平台：chatgpt-on-wechat / dify-on-wechat
...
3. **配置管理员**: 程序运行之后你需要修改`plugins/game/data/config.json`文件中的`admin_password`字段来配置您的管理员密码。，例如：
   ```json
   {
...
5. **开始游戏**：开始游戏(必须执行 `开机` 指令之后才能够正常交互)。
